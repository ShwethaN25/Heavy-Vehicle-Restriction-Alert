 ...
 *
 * @ingroup yolov5 --version 6.2 
 *
 * @author santi ram tiwari
 * Contact: stiwari@sparkcognition.com
 * Improvement : Removed the focus layer form the previous first layer and continued with the conv layers 
 *             : Added the sppf layers  
 *
 */
...
enum MODEL_TYPE {DEFAULT = 0, P6 = 1 , N = 2 , N6 = 3};
...
IPluginV2Layer *
ConstructModelV5n(INetworkDefinition *network, unsigned int maxBatchSize, float &gd, float &gw)
{
   ...
   ITensor *data = network->addInput(networkInfo.inputBlobName.c_str(),
    ...
    std::cout<<"Creating the v5n Model engine with parameters gd : "<<gd<<"  gw :"<<gw<<std::endl; 
...
IPluginV2Layer *
ConstructModelV5n6(INetworkDefinition *network, unsigned int maxBatchSize, float &gd, float &gw)
{
    std::cout << "Constructing model v5n" << std::endl;
    ...
    /* ------ yolov5 backbone------ */
    // Backbone
    auto conv0 = convBlock(network, weightMap, *data, get_width(32, gw), 6, 2, 1, "model.0");
    assert(conv0);
    auto conv1 = convBlock(network, weightMap, *conv0->getOutput(0), get_width(64, gw), 3, 2, 1, "model.1");
    auto c3_2 = C3(network, weightMap, *conv1->getOutput(0), get_width(64, gw), get_width(64, gw), get_depth(3, gd), true, 1, 0.5, "model.2");
    auto conv3 = convBlock(network, weightMap, *c3_2->getOutput(0), get_width(128, gw), 3, 2, 1, "model.3");
    auto c3_4 = C3(network, weightMap, *conv3->getOutput(0), get_width(128, gw), get_width(128, gw), get_depth(6, gd), true, 1, 0.5, "model.4");
    auto conv5 = convBlock(network, weightMap, *c3_4->getOutput(0), get_width(256, gw), 3, 2, 1, "model.5");
    auto c3_6 = C3(network, weightMap, *conv5->getOutput(0), get_width(256, gw), get_width(256, gw), get_depth(9, gd), true, 1, 0.5, "model.6");
    auto conv7 = convBlock(network, weightMap, *c3_6->getOutput(0), get_width(384, gw), 3, 2, 1, "model.7");
    auto c3_8 = C3(network, weightMap, *conv7->getOutput(0), get_width(384, gw), get_width(384, gw), get_depth(3, gd), true, 1, 0.5, "model.8");
    auto conv9 = convBlock(network, weightMap, *c3_8->getOutput(0), get_width(512, gw), 3, 2, 1, "model.9");
    auto c3_10 = C3(network, weightMap, *conv9->getOutput(0), get_width(512, gw), get_width(512, gw), get_depth(3, gd), true, 1, 0.5, "model.10");
    auto sppf11 = SPPF(network, weightMap, *c3_10->getOutput(0), get_width(512, gw), get_width(512, gw), 5, "model.11");

    // Head
    auto conv12 = convBlock(network, weightMap, *sppf11->getOutput(0), get_width(384, gw), 1, 1, 1, "model.12");
    ...
    auto c3_15 = C3(network, weightMap, *cat14->getOutput(0), get_width(768, gw), get_width(384, gw), get_depth(3, gd), false, 1, 0.5, "model.15");
    auto conv16 = convBlock(network, weightMap, *c3_15->getOutput(0), get_width(256, gw), 1, 1, 1, "model.16");
    ...
    auto c3_19 = C3(network, weightMap, *cat18->getOutput(0), get_width(512, gw), get_width(256, gw), get_depth(3, gd), false, 1, 0.5, "model.19");
    auto conv20 = convBlock(network, weightMap, *c3_19->getOutput(0), get_width(128, gw), 1, 1, 1, "model.20");
    ...
    auto c3_23 = C3(network, weightMap, *cat22->getOutput(0), get_width(256, gw), get_width(128, gw), get_depth(3, gd), false, 1, 0.5, "model.23");
    auto conv24 = convBlock(network, weightMap, *c3_23->getOutput(0), get_width(128, gw), 3, 2, 1, "model.24");
    ITensor *inputTensors25[] = {conv24->getOutput(0), conv20->getOutput(0)};
    auto cat25 = network->addConcatenation(inputTensors25, 2);
    auto c3_26 = C3(network, weightMap, *cat25->getOutput(0), get_width(512, gw), get_width(256, gw), get_depth(3, gd), false, 1, 0.5, "model.26");
    auto conv27 = convBlock(network, weightMap, *c3_26->getOutput(0), get_width(256, gw), 3, 2, 1, "model.27");
    ITensor *inputTensors28[] = {conv27->getOutput(0), conv16->getOutput(0)};
    auto cat28 = network->addConcatenation(inputTensors28, 2);
    auto c3_29 = C3(network, weightMap, *cat28->getOutput(0), get_width(768, gw), get_width(384, gw), get_depth(3, gd), false, 1, 0.5, "model.29");
    auto conv30 = convBlock(network, weightMap, *c3_29->getOutput(0), get_width(384, gw), 3, 2, 1, "model.30");
    ITensor *inputTensors31[] = {conv30->getOutput(0), conv12->getOutput(0)};
    auto cat31 = network->addConcatenation(inputTensors31, 2);
    auto c3_32 = C3(network, weightMap, *cat31->getOutput(0), get_width(1024, gw), get_width(512, gw), get_depth(3, gd), false, 1, 0.5, "model.32");
...
ConstructModel(INetworkDefinition* network, unsigned int maxBatchSize, float& gd, float& gw) {
    ...
    ITensor* data = network->addInput(networkInfo.inputBlobName.c_str(),
                                      DataType::kFLOAT,
                                      Dims3{3, networkInfo.height, networkInfo.width });
    assert(data);
...
ConstructModelP6(INetworkDefinition* network, unsigned int maxBatchSize, float& gd, float& gw) {
    ...
    /* ------ yolov5 backbone------ */
    // auto conv0 = convBlock(network, weightMap, *data, 3, get_width(64, gw), 3, "model.0", networkInfo.height, networkInfo.width);
    ...
    // This layer has been changed to the sspf in the new yolo v5--version6.2
...
BuildEngine(nvinfer1::IBuilder* builder, int model_type, float& gd, float& gw) {
    ...
    if(model_type == P6) {
        ...
        if(model_type == N)
        {
            std::cout<<"Making the  n model las ###################### "<<std::endl;
            yolo = ConstructModelV5n(network, networkInfo.batchsize, gd, gw);

        }
        else{
            if(model_type == N6)
            {
                yolo = ConstructModelV5n6(network, networkInfo.batchsize, gd, gw);
...
extern "C"
bool BuildCustomYOLOv5nEngine(nvinfer1::IBuilder * const builder,
        nvinfer1::IBuilderConfig * const builderConfig,
        const NvDsInferContextInitParams * const initParams,
        nvinfer1::DataType dataType,
        nvinfer1::ICudaEngine *& cudaEngine);
...
extern "C"
bool BuildCustomYOLOv5n6Engine(nvinfer1::IBuilder * const builder,
        nvinfer1::IBuilderConfig * const builderConfig,
        const NvDsInferContextInitParams * const initParams,
        nvinfer1::DataType dataType,
        nvinfer1::ICudaEngine *& cudaEngine);
...
extern "C" bool NvDsInferParseCustomYoloV5(
    ...
    for(auto& r : res) {
	    ...
	    oinfo.left    = static_cast<int>(r.bbox[0]-r.bbox[2]*0.5f);
        if (oinfo.left < 0){
            oinfo.left = 0;
        }
	    oinfo.top     = static_cast<int>(r.bbox[1]-r.bbox[3]*0.5f);
        if (oinfo.top < 0){
            oinfo.top = 0;
        }
	    oinfo.width   = static_cast<int>(r.bbox[2]);
        if (oinfo.left + oinfo.width > networkInfo.width){
            oinfo.width = networkInfo.width;
        }
	    oinfo.height  = static_cast<int>(r.bbox[3]);
        if (oinfo.top + oinfo.height > networkInfo.height){
            oinfo.height = networkInfo.height ;
        }
...
bool BuildCustomYOLOv5n6Engine(nvinfer1::IBuilder * const builder,
    ...
    cudaEngine = BuildEngine(builder, N6, gd, gw);
...
bool BuildCustomYOLOv5nEngine(nvinfer1::IBuilder * const builder,
    ...
    float gd = 0.33, gw = 0.25;

    assert(builder != nullptr);

    cudaEngine = BuildEngine(builder, N, gd, gw);
